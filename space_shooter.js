...
class Player extends Body {
	...
	update(delta_time) {
		...
		if (this.controller.move_x != 0) {
			this.velocity.x = 70 * this.controller.move_x;
		}

		if (this.controller.move_y != 0) {
			this.velocity.y = 70 * this.controller.move_y;
		}

		if (this.controller.move_x == 0) {
			if (this.velocity.x > 0)
				this.velocity.x =  Math.floor(9/10 * this.velocity.x);
			if (this.velocity.x < 0)
				this.velocity.x =  Math.ceil(9/10 * this.velocity.x);
		}

		if (this.controller.move_y == 0) {
			if (this.velocity.y > 0)
				this.velocity.y =  Math.floor(9/10 * this.velocity.y);
			if (this.velocity.y < 0)
				this.velocity.y =  Math.ceil(9/10 * this.velocity.y);
		}

		// if (this.controller.action_1 == 1) {
		// 	new Projectile();
		// }
...
class Enemy extends Body {
	...
	update(delta_time) {
		...
		// clip to screen
		if (this.position.y > config.canvas_size.height + this.half_size.height)
			super.remove()
		// this.position.x = Math.min(Math.max(0, this.position.x), config.canvas_size.width);
		// this.position.y = Math.min(Math.max(0, this.position.y), config.canvas_size.height);
	}
...
class EnemySpawner {
	...
	update(delta_time) {
		...
		if (this.timer > 5) {
			console.log("Spawn some enemies!");
			new Enemy();
			new Enemy();
			new Enemy();
			new Enemy();
